...
##
## Mirror of `composite-coverage` (COMP-05) and `synastry-coverage` (SYN-05).
## Same two-step pattern to avoid the NumPy `_NoValueType` reload bug
## triggered when coverage.py uses `source=ketu.returns` (sub-package).
## With `source=ketu` (full package) coverage works cleanly. We run
## pytest once with the project-wide `ketu` source (no `--cov-fail-under`
## since most modules are unexercised by tests/returns/), then scope
## the threshold check to `ketu/returns/*` via `coverage report
## --include`.
returns-coverage:
