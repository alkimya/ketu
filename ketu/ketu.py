...
decoder = vectorize(lambda x: x.decode('UTF-8'))
...
def body_index(swe_id):
    """Return the index of the body in bodies with a given swe_id.

    Parameters:
    swe_id (int): The swe_id of the body to find.

    Returns:
    int: The index of the body in the bodies array.
    """
    return nonzero(bodies.swe_id == swe_id)[0][0]


def body_name(swe_id):
    """Return the body name"""
    return decoder(bodies.name[body_index(swe_id)])


def body_orb(swe_id):
    """Return the body orb of influence of b_id"""
    return bodies.orb[body_index(swe_id)]
...
def maspect_index(aspect):
    ...
    return nonzero(maspects["angle"] == aspect)[0][0]


def dd_to_dms(dec):
    """Return degrees, minutes, seconds from degrees decimal"""
    mins, secs = map(int, divmod(dec * 3600, 60))
...
def julian_to_utc(julian, zoneinfo):
    ...
    return datetime(year, month, day, hour, minute, second, tzinfo=ZoneInfo("UTC")).astimezone(
        zoneinfo
    )
...
def get_chart(jdate, l_bodies=bodies):
    ...
    chart = fromiter([properties(jdate, body_id) for body_id in swe_id], dtype=datatype).view(
        recarray
    )
    return chart
...
def get_angle(chart, body1, body2):
    ...
    c_angle = array(
        (*jdate, body1, body2, *lon1, *lon2, *vlon1, *vlon2, *angle),
        dtype=datatype,
    )
    return c_angle
...
def get_aspects(chart):
    ...
    return sort(
        fromiter(gen_aspects(chart), dtype=datatype).view(recarray),
        order="score",
    )
...
def print_positions(jdate):
    ...
    for props in ndenumerate(chart):
        ...
        print(f"{body_name(swe_id):10}: " f"{signs[sign]:15}{degs:>2}º{mins:>2}'{secs:>2}\"{retro}")
