...
class TestTimeConversions:
    ...
    def test_local_to_utc(self):
        ...
        utc_naive = ketu.local_to_utc(self.day_one)
        assert utc_naive.tzinfo == ZoneInfo("UTC")
...
class TestEdgeCases:
    ...
    def test_extreme_dates(self):
        ...
        future_jday = ketu.utc_to_julian(future_date)
        assert future_jday > 2488000
        
        # Far past  
        past_date = datetime(1900, 1, 1)
        past_jday = ketu.utc_to_julian(past_date)
        assert past_jday < 2415000
        
        # Should still calculate positions
        sun_long = ketu.long(future_jday, 0)
        assert 0 <= sun_long <= 360
