...
**Latest shipped:** v1.8.0 (PyPI 2026-06-19) — <https://pypi.org/project/ketu/1.8.0/>

v1.8 added **declination speed** (`body_decl_speed`, dδ/dt deg/day) as a first-class chart field: `CHART_DTYPE` grows to 16 fields (the new field appended at index 8 for all 14 bodies, inherited across synastry / composite / returns — composite speed derived from the composite's own frozen chart, not the parent midpoint), a public standstill threshold `DECL_STANDSTILL_EPS = 0.001` exported from `ketu.calculations`, and a chart-level `is_ascending_declination_chart(chart)` helper returning int8 `{+1, 0, −1}` per body (montant/descendant/standstill) — distinct from the v1.5 scalar `is_ascending_declination(jdate, body)`. The finite difference uses the package-wide Δt = 0.01 d idiom (no new API surface). The library-design boundary is explicit: Ketu computes all the astronomy (including the FD and the standstill threshold); downstream consumers read a field and display it, computing no astronomy of their own. Engine shipped Phase 40 (DSPD-01..06, verifier 6/6); docs en+fr + the MINOR-1.8.0 release shipped Phase 41 (DSPD-07, REL-01). A full name-clean sweep removed every private-project reference from shipped/rendered EN+FR artifacts (generic "downstream consumers" phrasing) while preserving celestial-body names; FR `.mo` recompiled (no English fallback); EN+FR Sphinx builds at the 1-warning baseline. MINOR-not-patch because the dtype byte layout grows (named access safe; positional / `.view()` consumers must re-pin). Human go/no-go relecture-validation honoured before publish; post-publish fresh-venv smoke FROM PyPI green; runtime stays pure NumPy.

**Prior:** v1.7.0 (PyPI 2026-06-15) — <https://pypi.org/project/ketu/1.7.0/>

v1.7 gave the three fictitious points — Rahu (10), Ketu (11), Lilith (12) — a **2° aspect orb** (was `0°`) in the single-source `core.bodies` table, so they finally enter aspect detection. With `orb=0` the orb formula `(orb_b1 + orb_b2)/2 × coef` collapsed for two zero-orb points (only an exact-separation match), and Lilith could never aspect the nodes — surfaced by the Rahu (frontend) project, whose node/Lilith aspect grid rendered empty. Now point↔point conjunction = 2° and point↔planet = the mean (e.g. Rahu↔Sun = (2+12)/2 = 7°); every consumer (`get_orb`, `synastry_orb_limit`, cycles, composite, CLI) inherits the new value data-driven, no per-consumer edit. A shared `_is_tautological_node_opposition` helper, wired into all four public natal/scalar emit paths, suppresses ONLY the simultaneous `(Rahu, Ketu)` + `Opposition` detection — the permanent 180° artefact a non-zero orb creates (Ketu = South Node, exact opposite of Rahu by construction); Rahu and Ketu stay fully active for every other aspect and pair. Synastry was in scope: the `orb=0` oracles were rewritten (0.0→1.0 for the point self-pairs) and a full ~40-file regression sweep over every Rahu/Ketu/Lilith-referencing test passed, with two new CLI Rahu detections (Sun-Rahu Quincunx, Venus-Rahu Trine) deliberately pinned. Documented en + fr (the 2° orb, the filter rationale, the MINOR-not-patch note for Kala). Shipped as a **MINOR** bump (1.7.0), not a patch, because aspect results change for consumers. Verifier PASSED 4/4; runtime stays pure NumPy.

**Prior:** v1.6.0 (2026-06-04) added the additive `ketu.declination` subpackage — detection of **parallel** and **contra-parallel** aspects on the equatorial declination axis (δ): `find_declination_aspects(body_decl)` (scalar → `DECLA_ASPECT_DTYPE` upper-triangle P/CP pairs) + `declination_aspect_masks((S,14))→(S,91)` batch, body-derived orbs (`DECLA_COEF=1/12`, `MIN_DECL_ORB=0.5°`), docs en+fr. Fully additive: `CHART_DTYPE` byte-identical (companion function), `core.aspects` + fingerprints unchanged. v1.5.0 (2026-06-04) promoted equatorial declination δ to a first-class, vectorizable quantity — `declination` / `declination_velocity` / `is_ascending_declination` (biodynamic montant/descendant) / `is_out_of_bounds` in `ketu.calculations`, a `body_decl` field added to `CHART_DTYPE`, plus the dynamic-harmonics debt paid down (`H{h}-{k}` naming contract, `find_aspect_timing` `dyn_coef=`, CLI `--harmonics h7`). v1.4.0 (2026-06-03) made aspect harmonics open-ended — `generate_harmonic_aspects(h)` for ANY integer harmonic wired through the full detection chain via `dynamic_specs=`, the frozen `core.aspects` table + fingerprints byte-identical; Chiron range widened to 1900–2100 and its orb corrected 0°→4°; docs (en+fr) recentred on the 180°-division default. v1.3.0 (2026-06-01) embedded Chiron as the 14th body (Chebyshev `.npz`, pure-NumPy runtime), hardened the engine to 100% coverage, and made the aspect engine data-driven (default = TRADITIONAL 7). The 13→14 body shift deliberately broke the frozen-body-count ratchet and the internal Ketu↔Kala positional contract (Kala adapts).
...
**v1.7 Fictitious-Point Orbs** (full requirements archived in `.planning/milestones/v1.7-REQUIREMENTS.md`):

- ✓ Rahu/Ketu/Lilith orb `0°→2°` in the single-source `core.bodies` table; all consumers (`get_orb`, `synastry_orb_limit`, cycles, composite, CLI) inherit data-driven with no per-consumer edit; point↔point conjunction = 2°, point↔planet = mean (Rahu↔Sun = 7°) — v1.7.0 (ORB-01)
- ✓ Shared `_is_tautological_node_opposition` helper wired into all four public natal/scalar emit paths; suppresses ONLY the simultaneous `(Rahu, Ketu)` + `Opposition` detection (the permanent 180° artefact); Rahu and Ketu stay fully active for every other aspect and pair — v1.7.0 (ORB-02)
- ✓ Synastry inherits the new orb; `orb=0` oracles rewritten (0.0→1.0 for point self-pairs in `test_orbs.py` + idempotent-modes oracle); full ~40-file regression sweep green; two new CLI Rahu detections (Sun-Rahu Quincunx, Venus-Rahu Trine) deliberately pinned — v1.7.0 (ORB-03)
...
- **HARMF-01** — Rich `--harmonics` CLI grammar: multi-harmonic (`h7,h11`) and preset+harmonic mixing (`traditional,h7`). v1.5 shipped only the Tight single-token form; v1.6/v1.7 stayed off it. Remains a future candidate.
- **Declination synastry / dedicated CLI surface (DECLA-F1)** — v1.6 shipped in-orb detection only (no applying/timing/synastry/CLI for declination aspects). Natural follow-ups if demand surfaces.
- **Configurable per-call point orb** — override `core.bodies` at runtime. v1.7 set one global value (2°); a runtime override is a separate, larger config-surface decision with no current demand.

**Next direction:** the **Rahu** UI project (separate repo `~/workspace/rahu`, consumes `ketu` from PyPI, FastAPI + SvelteKit + D3/SVG) — work happens THERE, not in this repo. v1.7 unblocked it from the engine side (with `orb=0` the points never formed aspects, so the frontend rendered an empty node/Lilith grid). No further bodies, houses, or parts are planned for the Ketu engine; a Rust rewrite stays explicitly rejected.
...
- Configurable per-call point orb (runtime override of `core.bodies`) — v1.7 set one global value (2°); a runtime override is a separate, larger config-surface decision with no current demand
- Changing planet orbs (Sun..Chiron) — v1.7 touched the three fictitious points only; the Abu Ma'shar / Al-Biruni planet orbs stay frozen
...
| Fictitious-point orb `0°→2°` (single-source `core.bodies`), not a per-call override | One edit propagates to every consumer; matches the existing Chiron-orb single-source pattern; a runtime override is a separate larger config decision with no demand | ✓ Good (v1.7) — all consumers inherit data-driven; no per-consumer edit needed |
