# Requirements Archive: v1.2 Astrologie relationnelle et prédictive

**Archived:** 2026-05-28
**Status:** SHIPPED — 37/37 requirements complete

A fresh `.planning/REQUIREMENTS.md` is created at the start of the next milestone (`/gsd:new-milestone`).
...
- [x] **OPS-02** : `numpydoc validate` wiré dans CI sur les modules publics ; warnings non-bloquants au début, gate activé en fin de milestone (flip BLOCKING Phase 20)
- [x] **OPS-03** : Workflows refresh — `actions/checkout@v5+`, `actions/setup-python@v6+`, `actions/upload-artifact@v5+` (Node.js 24) ; tous les warnings Node 20 supprimés *(Phase 20, 2026-05-28)*
- [x] **OPS-04** : `fr/CHANGELOG.md` créé (synthétisé depuis l'anglais, pas double-maintenu) OU la référence aspirationnelle retirée — décision finale documentée *(Phase 20, 2026-05-28)*
- [x] **OPS-05** : v1.2 release published on PyPI as `ketu==1.2.0` via OIDC trusted publishing ; GitHub release avec sdist + wheel ; CHANGELOG `[1.2.0]` + UPGRADING.md migration recipes (additive only) *(Phase 20, 2026-05-28)*
...
- **5 Hermetic Lots restants** (Eros, Necessity, Courage, Victory, Nemesis) — variantes de tradition concurrentes ; registry les absorbe additivement
- **Transits / progressions / directions** — techniques prédictives au-delà du solar/lunar return ; v1.3 ou v1.4
...
- **Transits / progressions** — return charts uniquement pour v1.2 ; transits = continuous time-series, problème de forme différent
...
- **scipy / autres deps runtime** — pure-NumPy contract reste non-négociable (cf. CLAUDE.md). Bisection / Newton step en NumPy maison pour les returns.
...
**Outcome note:** Lunar return was listed "defer post-`solar_return`" at definition time but was pulled INTO v1.2 (Phase 18) since `_solve_return` generalized cleanly across Sun and Moon. The Out of Scope boundary was relaxed mid-milestone with the requirement set extended (LRET-01..05 + RET-06 added to the original RET-01..05).
...
| HOU2-01   | Phase 15 | ✓ Done  |
| HOU2-02   | Phase 15 | ✓ Done  |
| HOU2-03   | Phase 15 | ✓ Done  |
| HOU2-04   | Phase 15 | ✓ Done  |
| HOU2-05   | Phase 15 | ✓ Done  |
...
| OPS-01    | Phase 13 | ✓ Done  |
| OPS-02    | Phase 13 | ✓ Done  |
| OPS-03    | Phase 20 | ✓ Done  |
| OPS-04    | Phase 20 | ✓ Done  |
| OPS-05    | Phase 20 | ✓ Done  |

**Coverage:** 37/37 v1.2 requirements mapped and complete — no orphans, no double-mappings.

---

*Defined: 2026-05-08 by Sophie Chen during `/gsd-new-milestone`. Pre-research docs (`v1.2-SCOPE.md`, `v1.2-OPEN_QUESTIONS.md`) consumed and superseded by this file. Research path skipped (decided non-breaking minor with prior architectural decisions already captured). Traceability filled by `gsd-roadmapper` 2026-05-08 after roadmap approval. Archived 2026-05-28 at milestone completion.*
